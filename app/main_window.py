...
class Window(Frame):
    ...
    def __init__(self, root, app): #construtor recebe a rota
        super().__init__()
        self.root = root
        self.app = app
        self.db = Database()
        ...
        self.login_button = Button(self.frame, text="Login", cursor='hand2', font="sylfaen 15 bold", bd=5, relief="raised",
                            bg='#743913', command=lambda: Login(self.root, self.app).login()) #criando botão de login que vai abrir a janela para inserir os dados de autenticação
