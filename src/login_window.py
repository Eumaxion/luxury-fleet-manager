...
from src.login_top import Login
...
class Window(Frame):
    ...
    def login_window(self): #construtor recebe a rota
        self.frame_inicial = Frame(self.master, bd=4, bg='#743913', highlightthickness=3)
        ...
        self.login_button = Button(self.frame, text="Login", cursor='hand2', font="sylfaen 15 bold", bd=5, relief="raised",
                            bg='#743913', command=lambda: Login(self.root, self.controller.mostrar_menu)) #criando botão de login que vai abrir a janela para inserir os dados de autenticação
        self.cadastrar = Button(self.frame, text="Cadastrar usuário", cursor='hand2', font="sylfaen 15 bold",
                                bd=5, relief="raised", bg='#743913', command=lambda: Sign_up())
